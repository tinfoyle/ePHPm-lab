...
The worker process logs also emitted repeated `REST_REQUEST already defined` warnings. I did not run the worker browse benchmark after that failure. Its short response times would not make it a valid WooCommerce result while ordinary cart behavior is broken.
