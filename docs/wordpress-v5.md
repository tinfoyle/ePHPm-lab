...
Follow-up tracing identified the immediate cause: WooCommerce registers its `add_to_cart_action()` handler on WordPress's `wp_loaded` action, but the persistent worker adapter boots WordPress once and does not replay `wp_loaded` for each request. The `?add-to-cart=` request therefore rendered a normal `200` storefront page instead of performing the add-to-cart redirect or setting the WooCommerce session cookie. See [the worker investigation](wordpress-worker-investigation.md). I did not run the worker browse benchmark after that failure. Its short response times would not make it a valid WooCommerce result while ordinary cart behavior is broken.
