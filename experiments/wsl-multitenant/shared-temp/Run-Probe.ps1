...
if (!$CollectOnly) {
    $payload = Join-Path $cache 'shared-temp-payload.tar'
    & tar.exe -cf $payload --exclude=shared-temp/results --exclude=__pycache__ -C (Split-Path $PSScriptRoot) shared-temp
    ...
    & wsl.exe -d $Name -u root -- timeout 60s nsenter --net=/run/netns/ephpm-lab -- python3 /opt/ephpm-lab/shared-temp/run.py
    if ($LASTEXITCODE -ne 0) { throw 'Experiment failed; inspect guest results before retrying' }
}
if (!$RunId) {
    $runs = & wsl.exe -d $Name -u root -- ls -1 /var/lib/ephpm-lab/shared-temp
...
& wsl.exe -d $Name -u root -- test -f "/var/lib/ephpm-lab/shared-temp/$RunId/REPORT.md"
...
$p = Wsl-Process "tar -czf - -C /var/lib/ephpm-lab/shared-temp $RunId"
...
Write-Host "Report: $results/$RunId/REPORT.md"
